"""Background-job runner for `/write` (the agentic literature-review pipeline).

LLM-only (Ollama + Qdrant, both reachable from the backend), so it runs here in the lean
backend. Single-worker (serialized) in-process executor; job state persisted to a volume so
the UI can poll across reloads; stale running jobs are failed on startup. Mirrors the proven
pattern in `services/compare/jobs.py`.
"""
...
JOBS_DIR = Path(os.getenv("WRITEUP_JOBS_DIR", "/workspace/data/writeup_jobs"))
JOBS_DIR.mkdir(parents=True, exist_ok=True)

_executor = ThreadPoolExecutor(max_workers=1)  # one draft at a time
_jobs = {}
...
def start_job(topic, doc_ids, dimensions):
    ...
    with _lock:
        _jobs[job_id] = job
        _persist(job)
    _executor.submit(_run, job_id, topic, doc_ids, dimensions)
    return job_id
